// Adapted from https://github.com/kbob/shaderboy
...
void mainCube( out vec4 Q,vec3 U )
{
    ...
    Q.rg += smoothstep(.25, .2, length(v.xy-v.z*vec2(cos(3.*iTime/ISPEED), sin(3.*iTime/ISPEED))));
    Q.gb += smoothstep(.25, .2, length(v.xy-v.z*vec2(-cos(3.*iTime/ISPEED), -sin(3.*iTime/ISPEED))));
    Q *= exp(-length(U));
}
